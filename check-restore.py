...
def list_size(debug, username, password, authurl, authversion,
    ...
    for page in list_parts_gen:
        if page['success']:
            for item in page['listing']:
                ...
                if size == 0 and item['content_type'] == 'text/directory':
                    logger.debug(u'Skipping {0} because size is 0 and is text/directory'.format(name))
                    continue
    ...
    if exit_code:
        ...
        for f in missing_files:
            print(u'swift download {0} \'{1}\''.format(container, f))
